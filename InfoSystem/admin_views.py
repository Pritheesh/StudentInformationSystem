...
def upload_info(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            thread = threading.Thread(target=insert_info, args=(new_doc,), kwargs={})
...
def upload_results(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            new_doc.save()
            # book = xlrd.open_workbook(new_doc.docfile.name)
            thread = threading.Thread(target=insert_results, args=(new_doc, ei), kwargs={})
            thread.setDaemon(True)
            thread.start()
            return HttpResponseRedirect(reverse('links'))
    else:
